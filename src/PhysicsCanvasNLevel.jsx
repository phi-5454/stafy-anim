...
export default function PhysicsCanvasNLevel({ data }) {
  ...
  const distributeQuanta = useCallback(
    (n, target, nSystemTarget) => {
      ...
      let stars =
        Math.floor(energyQuanta) +
        (Math.random() < energyQuanta - Math.floor(energyQuanta) ? 1 : 0);
  ...
  useEffect(() => {
    ...
    const simulate = () => {
      ...
      nSystemColor.current = makeLoneSystemColor(
        nSystemState.current,
        N,
        energyQuanta,
      );
    ...
    <>
      <div className="flex flex-wrap w-full">
        ...
        >
          <div className="flex">
            <div className="w-1/2">
              <p className="text-2xl italic text-nowrap">Heat bath</p>
        ...
        <div className="m-auto basis-full sm:basis-1/2 aspect-[4/3]">
          ...
          <div className="columns-2">
            <p className="text-right">{"Mean: "}</p>
            <p className="text-left">{meanInd(pdata).toFixed(4)}</p>
          </div>
        </div>
      </div>
    </>
